...
const myInformation = ['Marina', 4, 'november', 'scorpio'];
...
let extraRussianCities = ['Санкт-Петербург', 'Москва', 'Екатеринбург'];
let result = [...extraRussianCities, ...russianCities]
console.log(result);
...
let filterEvenNumbers = numbers.filter(function(item) {
    return item % 2 == 0;
});
...
//Задание 12
//Напишите функцию sortAndPrint, которая принимает копию массива randomNumbers, numbers с помощью оператора spread и сортирует его в порядке возрастания с использованием метода sort(). Затем выведите отсортированный массив чисел в консоль.
let randomNumbers = [72, 45, 19, 84, 37, 56, 91, 23, 68, 10, 98, 32];

let newRowOfNumbers = [...randomNumbers, ...numbers];

const sortAndPrint = newRowOfNumbers.sort(function (a, b) {
    return a - b;
});
